...
#| fig-align: "center"
#| fig-cap:
#|  - "Length of tooth and supplement"
#|  - "Length of tooth and dosage "
...
```{r}
t.test(len~supp, data = ToothGrowth, alternative = c("two.sided"), mu = 0, paired = FALSE, var.equal = FALSE, conf.level = 0.95)
```
***Interpretation: *** 

1. Since the P value(0.06063) is greater than 0.05, it means that we fail to reject the null hypothesis meaning that we don't have enough evidence to conclude that there is a difference between the tooth length for the OJ and the VC supplement delivery method.
2. We are 95% confident that the difference in tooth growth between group OJ and group VC is between -0.17 and 7.57. Since the confidence interval contains 0 it means that there is not enough evidence to conclude a difference between the tooth length for the OJ and the VC supplement delivery method
